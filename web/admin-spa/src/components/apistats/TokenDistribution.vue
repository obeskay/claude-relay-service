  ...
  <div class="card p-4 md:p-6">
    ...
    >
      <span class="flex items-center">
        <i class="fas fa-coins mr-2 text-sm text-warning md:mr-3 md:text-base" />
        {{ t('apistats.token.distribution') }}
      </span>
    ...
    <div class="space-y-2 md:space-y-3">
      <div class="flex items-center justify-between">
        <span class="flex items-center text-sm text-muted-foreground md:text-base">
          <i class="fas fa-arrow-right mr-1 text-xs text-success md:mr-2 md:text-sm" />
          {{ t('apistats.token.input') }}
        </span>
      ...
      <div class="flex items-center justify-between">
        <span class="flex items-center text-sm text-muted-foreground md:text-base">
          <i class="fas fa-arrow-left mr-1 text-xs text-primary md:mr-2 md:text-sm" />
          {{ t('apistats.token.output') }}
        </span>
        <span class="text-sm font-medium text-foreground md:text-base">{{
    ...
    <div class="mt-3 border-t border-gray-200 pt-3 dark:border-gray-700 md:mt-4 md:pt-4">
      <div class="flex items-center justify-between font-bold text-foreground">
        <span class="text-sm md:text-base">{{
          statsPeriod === 'daily'
            ? t('apistats.token.total_today')
            : t('apistats.token.total_this_month')
        }}</span>
