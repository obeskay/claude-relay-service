    ...
    >
      <div class="mt-3">
        <div class="mb-4 flex items-center justify-between">
          <h3 class="text-lg font-medium text-gray-900">{{ t('user.create_modal.title') }}</h3>
        ...
        <form class="space-y-4" @submit.prevent="handleSubmit">
          <div>
            <label class="block text-sm font-medium text-gray-700" for="name">
              {{ t('user.create_modal.name_label') }}
            </label>
            <input
              id="name"
              v-model="form.name"
              class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-primary focus:ring-ring sm:text-sm"
              :disabled="loading"
              :placeholder="t('user.create_modal.name_placeholder')"
              required
              type="text"
            />
          </div>
          ...
          <div>
            <label class="block text-sm font-medium text-gray-700" for="description">
              {{ t('user.create_modal.description_label') }}
            </label>
            <textarea
              id="description"
              ...
              class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-primary focus:ring-ring sm:text-sm"
              :disabled="loading"
              :placeholder="t('user.create_modal.description_placeholder')"
              rows="3"
            ></textarea>
          </div>
          ...
          <div class="flex justify-end space-x-3 pt-4">
            ...
            >
              <span v-if="loading" class="flex items-center">
                ...
                {{ t('user.create_modal.creating') }}
              </span>
              <span v-else>{{ t('user.create_modal.create_button') }}</span>
            </button>
          </div>
        </form>
          ...
          <div class="flex items-start">
            ...
            <div class="ml-3 flex-1">
              <h4
                ...
                v-html="t('user.create_modal.success_title')"
              ></h4>
              <div class="mt-3">
                <p
                  class="mb-2 text-sm text-green-700"
                  v-html="t('user.create_modal.success_message')"
                ></p>
              ...
              <div class="mt-4 flex justify-end">
                <button
                  ...
                  @click="handleClose"
                >
                  {{ t('user.create_modal.done_button') }}
                </button>
              </div>
            </div>
          </div>
...
const handleSubmit = async () => {
  if (!form.name.trim()) {
    error.value = t('user.create_modal.error_required')
    return
  }
  ...
  try {
    ...
    if (result.success) {
      ...
      showToast(t('user.create_modal.toast_success'), 'success')
    } else {
      error.value = result.message || t('user.create_modal.error_create')
    }
  } catch (err) {
    console.error('Create API key error:', err)
    error.value = err.response?.data?.message || err.message || t('user.create_modal.error_create')
  } finally {
    loading.value = false
  }
}
...
const copyToClipboard = async (text) => {
  try {
    ...
    showToast(t('user.create_modal.toast_copied'), 'success')
  } catch (err) {
    console.error('Failed to copy:', err)
    showToast(t('user.create_modal.toast_copy_failed'), 'error')
  }
}
